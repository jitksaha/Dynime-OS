// Phase 5 — payment-initiate ported to a Worker.
// Source: supabase/functions/payment-initiate/index.ts (1262 lines)
...
//   - gateway creds         -> getPaymentGatewayConfig(sql, gateway) (DB-driven secrets)
//
// This is the multi-gateway router: subscription / addon / wallet_topup, each
// dispatching to one of SSLCommerz, Stripe (one-time + recurring), bKash, Dodo,
// Razorpay, 2Checkout. The per-gateway handlers, currency conversion, amounts,
// HMAC signing (2Checkout), and pending-record writes are preserved verbatim.
//
// Sibling-callback URLs that previously pointed at ${SUPABASE_URL}/functions/v1
// now point at ${APP_URL}/functions/v1 (the Workers functions gateway), since the
// gateways call those callbacks server-side after redirect.
...
async function convertForGateway(
  ...
  const cacheRows = await withSession(sql, SERVICE, (tx) =>
    tx`SELECT rates FROM public.exchange_rate_cache
       WHERE base_currency = 'USD'
       ORDER BY fetched_at DESC
       LIMIT 1`);
  const cacheRow = cacheRows[0] as { rates?: Record<string, number> } | undefined;
  ...
  const countriesRows = await withSession(sql, SERVICE, (tx) =>
    tx`SELECT value FROM public.platform_settings WHERE key = 'enabled_countries' LIMIT 1`);
  const countriesRow = countriesRows[0] as { value?: any[] } | undefined;
...
async function logGatewayHealth(
  ...
  try {
    await withSession(sql, SERVICE, (tx) =>
      tx`INSERT INTO public.gateway_health_logs ${tx({
...
export async function handler(req: Request, env: Env): Promise<Response> {
  ...
  try {
    ...
    const [gwConfig, profileRows] = await Promise.all([
      getPaymentGatewayConfig(sql, gateway),
    ...
    const creds: Record<string, string> = (rawCreds.sandbox || rawCreds.live)
      ...
      : rawCreds;
    const isSandbox = !!(gwConfig as any).is_sandbox;
    const processingCurrency = (gwConfig as any).processing_currency as string | null | undefined;
    ...
    // Sibling-function callbacks now resolve to the Workers functions gateway.
    ...
    if (body.saved_method_id) {
      const savedRows = await withSession(sql, SERVICE, (tx) =>
        tx`SELECT * FROM public.saved_payment_methods
           WHERE id = ${body.saved_method_id} AND user_id = ${user.id} AND is_active = true
           LIMIT 1`);
      ...
      if (savedMethod.gateway_key === "bkash" && savedMethod.agreement_id) {
        const bkashCreds = (rawCreds.sandbox || rawCreds.live)
          ? (isSandbox ? (rawCreds.sandbox || {}) : (rawCreds.live || {}))
          : rawCreds;
        const bkashBase = isSandbox
          ? "https://tokenized.sandbox.bka.sh/v1.2.0-beta/tokenized/checkout"
          : "https://tokenized.pay.bka.sh/v1.2.0-beta/tokenized/checkout";

        const chargeAmount = body.checkout_amount || body.amount || 0;
        const chargeCurrency = body.checkout_currency || "BDT";
        ...
        const callbackUrl = `${functionsUrl}/bkash-tokenize?${callbackMeta.toString()}`;
          ...
          if (body.purpose === "subscription" && body.plan_id) {
            await withSession(sql, SERVICE, (tx) =>
              tx`INSERT INTO public.tenant_subscriptions ${tx({
                tenant_id: tenantId, plan_id: body.plan_id, billing_cycle: body.billing_cycle,
      ...
      if (savedMethod.gateway_key === "stripe" && savedMethod.token) {
        ...
        const chargeAmount = body.checkout_amount || body.amount || 0;
        const chargeCurrency = (body.checkout_currency || body.currency || "usd").toLowerCase();
    ...
    if (purpose === "subscription") {
      ...
      const { amount, currency: chargeCurrency } = await convertForGateway(sql, rawAmount, planCurrency, gateway, processingCurrency);
      const tranId = `TXN_${Date.now()}_${crypto.randomUUID().slice(0, 8)}`;

      if (gateway === "sslcommerz") {
        return await handleSSLCommerz(creds, isSandbox, {
          ...
          valueA: tenantId, valueB: plan_id, valueC: billing_cycle, valueD: siteUrl,
        }, async () => {
          await withSession(sql, SERVICE, (tx) =>
            tx`INSERT INTO public.tenant_subscriptions ${tx({
              tenant_id: tenantId, plan_id, billing_cycle, amount: rawAmount,
        ...
        if (billing_cycle === "lifetime") {
          return await handleStripeOneTime(creds, isSandbox, {
            ...
            userId: user.id, tenantId: tenantId, email: user.email || "",
        ...
        return await handleStripeSubscription(creds, isSandbox, {
        ...
        return await handleBkash(creds, isSandbox, {
          amount, currency: chargeCurrency, tranId,
        ...
        return await handleDodo(creds, isSandbox, {
          amount, currency: chargeCurrency, tranId,
          productName: `${plan.name} Plan - ${billing_cycle}`,
          customerName: profile.full_name || "Customer",
          customerEmail: user.email || "customer@example.com",
        ...
        return await handleRazorpay(creds, isSandbox, {
          amount, currency: chargeCurrency, tranId,
          productName: `${plan.name} Plan - ${billing_cycle}`,
          customerName: profile.full_name || "Customer",
          customerEmail: user.email || "customer@example.com",
        ...
        return await handle2Checkout(creds, isSandbox, {
          amount, currency: chargeCurrency, tranId,
          productName: `${plan.name} Plan - ${billing_cycle}`,
          customerName: profile.full_name || "Customer",
          customerEmail: user.email || "customer@example.com",
    ...
    if (purpose === "addon") {
      ...
      const [addonRows, activeSubRows] = await Promise.all([
        withSession(sql, SERVICE, (tx) =>
      ...
      if (!activeSubRows[0]) return jsonResp({ error: "An active subscription is required" }, 400);

      const existingRows = await withSession(sql, SERVICE, (tx) =>
        tx`SELECT id FROM public.tenant_addon_modules
           WHERE tenant_id = ${tenantId} AND module_name = ${addon.module_name}
             AND status IN ${tx(["active", "pending"])}
           LIMIT 1`);
      if (existingRows[0]) return jsonResp({ error: "You already have this module" }, 400);
      ...
      const { amount, currency: chargeCurrency } = await convertForGateway(sql, rawAmount, "USD", gateway, processingCurrency);
      ...
      if (gateway === "sslcommerz") {
        return await handleSSLCommerz(creds, isSandbox, {
          ...
          valueA: tenantId, valueB: addon.module_name, valueC: payment_type, valueD: siteUrl,
        }, onSuccess);
      } else if (gateway === "stripe") {
        return await handleStripeOneTime(creds, isSandbox, {
          amount, currency: chargeCurrency,
    ...
    if (purpose === "wallet_topup") {
      ...
      if (tenantId !== tenant_id) return jsonResp({ error: "Unauthorized tenant" }, 403);

      const { amount, currency: chargeCurrency } = await convertForGateway(sql, parsedAmount, "USD", gateway, processingCurrency);
...
async function handle2Checkout(
  ...
  // Generate HMAC signature for security (Web Crypto — preserved exactly).
