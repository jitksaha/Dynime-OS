// Ported from supabase/functions/ai-models-list/index.ts — lists chat-capable models
// for a provider (openai/anthropic/google), fetching live where possible with curated
// fallbacks. Already de-branded (no "lovable" provider). Behaviour preserved 1:1.
//   - getApiKey: createClient(SERVICE_ROLE) DB read  becomes  getPlatformSetting("ai_config")
...
function sortNewestFirst(_provider: string, ids: string[]): string[] {
  return [...ids].sort((a, b) => {
...
async function getApiKey(env: Env, provider: string): Promise<string | null> {
  // 1. Try DB (platform_settings.ai_config.api_keys)
  try {
    const sql = connect(env);
    const cfg = await getPlatformSetting<Record<string, any>>(sql, "ai_config");
    const k = (cfg as any)?.api_keys?.[provider];
    if (k) return k;
  } catch (_) {}
  // 2. Fallback to Worker secret
  const envMap: Record<string, string> = {
    openai: "OPENAI_API_KEY",
    anthropic: "ANTHROPIC_API_KEY",
    google: "GOOGLE_AI_API_KEY",
  };
  return (env as any)[envMap[provider]] || null;
}
