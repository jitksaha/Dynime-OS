// Ported from supabase/functions/kyb-verify/index.ts.
...
async function verifyWithAI(sql: Sql, env: Env, tenant: any, tenantId: string) {
  ...
  for (const [key, value] of Object.entries(docs)) {
    if (value && (value as any).path) {
      const signedUrl = await createSignedUrl((value as any).path);
      if (signedUrl) {
        imageUrls.push({ label: key, url: signedUrl });
      }
    }
  }
  ...
  try {
    ...
    if (decision === "approved") {
      await withSession(sql, SERVICE, (tx) =>
        tx`UPDATE public.tenants SET kyb_status = 'approved', kyb_verified_at = ${new Date().toISOString()} WHERE id = ${tenantId}`);
    } else if (decision === "rejected") {
      await withSession(sql, SERVICE, (tx) =>
        tx`UPDATE public.tenants SET kyb_status = 'rejected', kyb_rejection_reason = ${reason} WHERE id = ${tenantId}`);
    } else {
      // needs_review → mark as under_review
      await withSession(sql, SERVICE, (tx) =>
        tx`UPDATE public.tenants SET kyb_status = 'under_review' WHERE id = ${tenantId}`);
    }
...
async function verifyWithSumsub(sql: Sql, tenant: any, tenantId: string, config: VerificationConfig) {
...
export async function handler(req: Request, env: Env): Promise<Response> {
  ...
  try {
    const { tenant_id, method } = await req.json() as any;
    ...
    const rows = await withSession(sql, SERVICE, (tx) =>
      tx`SELECT * FROM public.tenants WHERE id = ${tenant_id} LIMIT 1`);
    const tenant = rows[0];
    ...
    let result;
    if (method === "sumsub") {
      result = await verifyWithSumsub(sql, tenant, tenant_id, config);
    } else {
      result = await verifyWithAI(sql, env, tenant, tenant_id);
    }
