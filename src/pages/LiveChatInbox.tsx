...
import { MessageCircle, Send, X, Bot, User, CheckCircle2, AlertTriangle, Search, RefreshCw, StickyNote } from "lucide-react";
...
const CANNED_RESPONSES = [
  ...
  "I understand your concern. Let me check and get back to you shortly.",
  ...
  return (
    <div className="space-y-4 animate-fade-in">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          <h1 className="text-xl sm:text-2xl font-bold text-foreground">Live Chat Support</h1>
          <p className="text-sm text-muted-foreground mt-1">Manage visitor conversations and AI-escalated chats</p>
        </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        {[
          { label: "Active Chats", count: conversations.filter((c) => c.status === "active").length, icon: MessageCircle, color: "text-primary" },
          { label: "Escalated", count: conversations.filter((c) => c.is_escalated && c.status === "active").length, icon: AlertTriangle, color: "text-warning" },
          { label: "AI Handled", count: conversations.filter((c) => !c.is_escalated && c.status === "active").length, icon: Bot, color: "text-info" },
          { label: "Closed", count: conversations.filter((c) => c.status === "closed").length, icon: CheckCircle2, color: "text-success" },
        ].map((s) => (
        ...
        <div className={cn("w-full lg:w-80 shrink-0 bg-card border border-border rounded-xl flex flex-col overflow-hidden", selectedId && "hidden lg:flex")}>
          <div className="p-3 border-b border-border space-y-2">
            ...
            <div className="flex gap-1">
              {(["all", "escalated", "active", "closed"] as const).map((f) => (
                <button key={f} onClick={() => setFilter(f)} className={cn("px-2 py-1 rounded-md text-xs font-medium transition-colors capitalize", filter === f ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:bg-accent")}>
                  {f}
                </button>
              ))}
            </div>
          ...
          <div className="flex-1 overflow-y-auto">
            {loading ? (
              ...
              filtered.map((c) => (
                <button key={c.id} onClick={() => setSelectedId(c.id)} className={cn("w-full text-left p-3 border-b border-border hover:bg-accent/50 transition-colors", selectedId === c.id && "bg-primary/5 border-l-2 border-l-primary")}>
            ...
            <>
              <div className="flex items-center justify-between px-4 py-3 border-b border-border">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">{selectedConvo?.visitor_email}</p>
                </div>
                <div className="flex items-center gap-1.5">
                  ...
                  {selectedConvo?.status === "active" && (
                    <button onClick={() => closeConversation(selectedId!)} className="px-3 py-1.5 rounded-md text-xs font-medium border border-input hover:bg-accent transition-colors">Close Chat</button>
                  )}
              ...
              {showNoteInput && (
                <div className="px-4 py-2 border-b border-border bg-warning/5 flex gap-2">
                  <input value={internalNote} onChange={(e) => setInternalNote(e.target.value)} onKeyDown={(e) => e.key === "Enter" && saveInternalNote()} placeholder="Add internal note..." className="flex-1 h-8 px-3 rounded-md border border-warning/30 bg-background text-sm focus:outline-none focus:ring-1 focus:ring-warning" />
                ...
                {messages.map((msg) => {
                  ...
                  return (
                    <div key={msg.id} className={cn("flex gap-2", msg.sender_type === "visitor" ? "justify-end" : "justify-start")}>
                      ...
                      <div>
                        <div className={cn("max-w-md rounded-xl px-3 py-2 text-sm", msg.sender_type === "visitor" ? "bg-muted text-foreground rounded-br-sm" : msg.sender_type === "agent" ? "bg-primary text-primary-foreground rounded-bl-sm" : "bg-secondary text-foreground rounded-bl-sm")}>
              ...
              {showCanned && selectedConvo?.status === "active" && (
                <div className="px-4 py-2 border-t border-border bg-accent/30 space-y-1 max-h-36 overflow-y-auto">
              ...
              {selectedConvo?.status === "active" && (
                <div className="p-3 border-t border-border">
                  <div className="flex gap-2">
                    <button onClick={() => setShowCanned(!showCanned)} className="h-9 px-2.5 rounded-lg border border-input text-muted-foreground hover:bg-accent transition-colors text-xs font-medium shrink-0" title="Quick replies">
                      ⚡
                    </button>
                    <input value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => e.key === "Enter" && !e.shiftKey && sendReply()} placeholder="Type your reply..." className="flex-1 h-9 px-3 rounded-lg border border-input bg-background text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-1 focus:ring-ring" />
                    <button onClick={sendReply} disabled={sending || !input.trim()} className="h-9 w-9 flex items-center justify-center rounded-lg bg-primary text-primary-foreground disabled:opacity-50 hover:opacity-90 transition-opacity">
