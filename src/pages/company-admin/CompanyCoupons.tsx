...
export default function CompanyCoupons() {
  ...
  useEffect(() => { fetchCoupons(); }, [profile?.tenant_id]);
  ...
  return (
    <div className="space-y-6 animate-fade-in max-w-5xl">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl sm:text-2xl font-bold text-foreground">Company Coupons</h1>
      ...
      {showForm && (
        <div className="bg-card border border-border rounded-xl p-5 space-y-4">
          <h2 className="text-sm font-semibold text-foreground">Create Customer Coupon</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            <div>
              <label className="block text-xs font-medium text-muted-foreground mb-1">Code</label>
              <input value={form.code} onChange={(e) => setForm({ ...form, code: e.target.value })} placeholder="WELCOME10" className="w-full h-9 rounded-lg border border-input bg-background px-3 text-sm" />
            </div>
            <div>
      ...
      {loading ? (
        ...
        <div className="text-center py-12 bg-card border border-border rounded-xl">
          ...
          <p className="text-sm text-muted-foreground">No coupons yet. Create one for your customers!</p>
        </div>
      ) : (
              ...
              <div className="flex items-center gap-3 min-w-0">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-xs text-muted-foreground mt-0.5">
                    {c.coupon_type === "percentage" ? `${c.value}% off` : `${cs}${c.value} off`}
                    {c.max_discount_amount ? ` (max ${cs}${c.max_discount_amount})` : ""}
